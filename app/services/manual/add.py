...
def validate(category: str, items: list[dict[str, Any]]) -> str | None:
    ...
    if category == "speaking_repeat":
        if not item["title"]:
            return "缺少场景标题"
        if len(item["sentences"]) != _REPEAT_SENTENCE_COUNT:
            return f"听后复述需要恰好 {_REPEAT_SENTENCE_COUNT} 句（当前 {len(item['sentences'])} 句）"
    elif category == "speaking_interview":
        if not item["title"]:
            return "缺少主题标题"
        if len(item["questions"]) != _INTERVIEW_QUESTION_COUNT:
            return f"互动面试需要恰好 {_INTERVIEW_QUESTION_COUNT} 题（当前 {len(item['questions'])} 题）"
    else:
        if not item["title"]:
            return "缺少标题"
